...
class InverseModel:
    def __init__(self, temperature, eui, energy_type='Energy type unknown', significance_threshold=0.1):
        ...
        if (np.size(eui) != np.size(temperature)):
            ...
            #print(self.hcp, self.ccp, self.temperature)
            ...
            if eui.min() != eui.max():
                self.base_min = eui.min() #0  # Baseload minimum
                self.base_max = eui.max() #np.inf  # Baseload maximum
            else:
                self.base_min = -np.inf #0  # Baseload minimum
            ...
            #print("intitial hcp and ccp",self.hcp,self.ccp)
            #return(self.hsl_min)
    ...
    def piecewise_linear(x, hcp, ccp, base, hsl, csl):
        ...
        #print("peicewise_liner parameters", hcp, ccp, base, hsl, csl)
    ...
    def fit(self):
        try:
            #print("fit parameter",self.hcp_min, self.ccp_min, self.base_min, self.hsl_min, self.csl_min,self.hcp_max, self.ccp_max, self.base_max, self.hsl_max, self.csl_max)
            try:
    ...
    def optimize_cp_limit(self, point):
        ...
        if point == "R":
            percentiles = [[i, i + 5] for i in np.arange(20, 90, 5)]
            #print("R", percentiles)
        else:
            percentiles = [[i, i + 5] for i in np.arange(10, 80, 5)]
            #print("L", percentiles)

        var = []
        #print('optimize cp limits start')
        for per in percentiles:
            ...
            #print("optimize_cp_limit", self.hcp_min, self.hcp_max, self.ccp_min, self.ccp_max)
            self.fit()
            # print(self.p)
            r2 = self.R_Squared()
        ...
        if point == "L":
            ...
            self.ccp_min = cp_limit_min  # Cooling change-point minimum
            self.ccp_max = cp_limit_max  # Cooling change-point maximum
        #print('optimize cp limits end', self.hcp_min, self.hcp_max, self.ccp_min, self.ccp_max)
        self.fit()
