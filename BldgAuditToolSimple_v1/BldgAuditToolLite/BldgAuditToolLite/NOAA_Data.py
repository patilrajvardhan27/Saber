...
class NOAAData:
    ...
    def download_weather_NOAA(self):
        ...
        try:
            ...
            self.v_T_F, self.v_T_C, self.df_new = self.process_downloaded_weather(self.weather_station_ID,self.start_year,self.end_year,self.v_start_dates,self.v_end_dates)
        except:
            try:
                ...
                self.weather_station_ID = self.weather_station_list[3]
                self.v_T_F, self.v_T_C, self.df_new = self.process_downloaded_weather(self.weather_station_ID, self.start_year, self.end_year, self.v_start_dates, self.v_end_dates)
        return(self.v_T_F, self.v_T_C, self.df_new)
    ...
    def aggregate_weather_month(self, df_daily, v_start_dates, v_end_dates):
        ...
        v_avg_period_T_F = v_avg_period_T_F[~np.isnan(v_avg_period_T_F)]
        v_avg_period_T_C = v_avg_period_T_C[~np.isnan(v_avg_period_T_C)]
        ...
        return (v_avg_period_T_F, v_avg_period_T_C, df_monthly_avg.dropna())
    ...
    def aggregate_weather_day(self, df_daily,v_start_dates, v_end_dates):    
        ...
        return (np.array(df_daily_avg['Avg_Temperature_F'].dropna()), np.array(df_daily_avg['Avg_Temperature_C'].dropna()),df_daily_avg.dropna())    
    ...
    def aggregate_weather_hour(self, df_daily,v_start_dates, v_end_dates):    
        ...
        return (np.array(df_temp['Avg_Temperature_F'].dropna()), np.array(df_temp['Avg_Temperature_C'].dropna()), df_temp.dropna())  
