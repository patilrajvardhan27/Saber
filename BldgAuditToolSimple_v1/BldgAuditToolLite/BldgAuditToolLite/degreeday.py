...
class DegreeDay:
    ...
    def heating_degree_day(self, mask = None):
        df_DDh = []
        df_DDh = pd.DataFrame({"OAT": self.oat})
        df_DDh["hcp"] = self.hcp
        df_DDh["Dates"] = self.df_new["Date"].values 
        ...
        for i, col in enumerate(degree_day_cols):
            ...
            x_vals = np.linspace(0, max(X) * 1.1, 100)  # Extend slightly beyond max X
            y_vals = intercept + slope * x_vals
    
            # Scatter plot for actual data points (Legend includes intercept & R² value)
            fig.add_trace(go.Scatter(
                x=X.flatten(), 
                y=Y, 
                mode='markers', 
                name=f"{col} (Intercept={intercept:.3f}, R²={r_squared:.2f})",
                marker=dict(size=6, opacity=0.7, color=colors[i])
            ))
    
            # Add best-fit line (matching scatter point color)
            fig.add_trace(go.Scatter(
                x=x_vals.flatten(), 
                y=y_vals.flatten(), 
                mode='lines', 
                line=dict(dash='solid', width=2, color=colors[i]),
                showlegend=False
            ))
    
        # Update layout for interactivity
        fig.update_layout(
            title="Interactive Degree Day Heating vs Fossil Fuel Consumption",
            xaxis_title="Degree Day Heating",
            yaxis_title="Fossil Fuel/sqft",
            legend_title="Degree Day Heating Variables (Intercept, Slope & R²)",
        ...
        return fig, best_intercept, best_numeric, best_slope
    ...
    def cooling_degree_day(self, mask = None):
        ...
        df_DDc["ccp"] = self.ccp
        df_DDc["Dates"] = self.df_new["Date"].values 
        ...
        for i, col in enumerate(degree_day_cols):
            ...
            # Generate trendline values, ensuring it extends to the y-axis
            x_vals = np.linspace(0, max(X) * 1.1, 100)
            y_vals = intercept + slope * x_vals
        ...
        # Update layout for interactivity
        fig.update_layout(
            title="Interactive Degree Day Cooling vs Electricity Consumption",
            xaxis_title="Degree Day Cooling",
            yaxis_title="Electricity/sqft",
            legend_title="Degree Day Cooling Variables (Intercept, Slope & R²)",
            template="plotly_white"
        )
